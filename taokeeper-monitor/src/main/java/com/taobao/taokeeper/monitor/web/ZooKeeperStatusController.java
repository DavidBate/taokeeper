...
public class ZooKeeperStatusController extends BaseController {
	...
	public ModelAndView showZooKeeperStatusPAGE(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			Map<String, ZooKeeperRTInfo> rtInfoMap = new HashMap<String, ZooKeeperRTInfo>();
			Map<String, SrvrInfo> srvrMap = new HashMap<String, SrvrInfo>();
			Map<String, ZooKeeperDelayInfo> delayMap = new HashMap<String, ZooKeeperDelayInfo>();
			SrvrInfo leaderInfo = new SrvrInfo();
			// 进行系统数据采集
			List<String> serverList = zooKeeperCluster.getServerList();
			if (null != serverList) {
				for (String server : serverList) {
					String ip = StringUtil.trimToEmpty(server.split(COLON)[0]);
					int port = NumberUtils.toInt(server.split(COLON)[1]);
					// 获取自检状态
					int statusType = GlobalInstance.getZooKeeperStatusType(ip);
					// ZooKeeperStatusV2 zooKeeperStatus =
					// GlobalInstance.getZooKeeperStatus(ip);
					ZooKeeperStatusV2 zooKeeperStatus = ZookeeperData.getZooKeeperStatus(ip, port);
					...
					rtInfoMap.put(ip, ZookeeperData.getRTInfo(ip, port));
					SrvrInfo srvr = ZookeeperData.getSrvr(ip, port);
					if (srvr == null) {
						continue;
					}
					if (zooKeeperStatus.isLeader()) {
						leaderInfo = srvr;
					}
					srvrMap.put(ip, srvr);
				}
				if (leaderInfo.getZxid() != null) {
					for (Entry<String, SrvrInfo> entry : srvrMap.entrySet()) {
						ZooKeeperDelayInfo delay = new ZooKeeperDelayInfo();
						delay.setInProcessTask(entry.getValue().getOutstanding());
			...
			// model.put("clusterRTStatsMap",
			// ZooKeeperRTCollectJob.getRtStatus().get(zooKeeperCluster.getClusterId()));
			// model.put("clusterRTStats",
			// ZooKeeperRTCollectJob.getClustRTStatus().get(zooKeeperCluster.getClusterId()));
			model.put("rtInfoMap", rtInfoMap);
			model.put("timeOfRTUpdate", GlobalInstance.timeOfUpdateRT);

			model.put("delayInfoMap", delayMap);
